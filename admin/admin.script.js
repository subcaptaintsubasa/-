...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const baseTypeMappings = { // For display purposes
        headShape: "頭の形",
        correction: "補正",
        color: "色",
        pattern: "柄"
    ...
    function clearAdminUI() {
        ...
        if (newEffectTypeSumCapGroup) newEffectTypeSumCapGroup.style.display = 'block'; // Default show for "sum"
    ...
    // --- Effect Unit Management ---
    // ... (No changes from previous admin.script.js) ...
    ...
    function renderEffectTypesForManagement() {
        ...
        effectTypesCache.forEach(effectType => {
            ...
            const sumCapText = (effectType.calculationMethod === 'sum' && typeof effectType.sumCap === 'number') ? ` (上限: ${effectType.sumCap})` : '';
            const div = document.createElement('div');
            div.classList.add('list-item');
            div.innerHTML = `
    ...
    function toggleSumCapInputVisibility(radios, sumCapGroup, sumCapInput) {
        if (!radios || !sumCapGroup || !sumCapInput) return;
    ...
    if(newEffectTypeCalcMethodRadios && newEffectTypeSumCapGroup && newEffectTypeSumCapInput){
    ...
    if(editingEffectTypeCalcMethodRadios && editingEffectTypeSumCapGroup && editingEffectTypeSumCapInput){
    ...
    function openEditEffectTypeModal(effectTypeData) {
        ...
        if (calcMethod === 'sum' && typeof effectTypeData.sumCap === 'number') {
    ...
    // --- Item Management ---
    // ... (Functions are largely the same, `renderItemsAdminTable` and `saveItemButton` were adjusted for `price`) ...
