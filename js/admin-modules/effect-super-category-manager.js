...
export function _renderEffectSuperCategoriesForManagementInternal(superCategoriesCache) {
    ...
    const listContent = buildEffectSuperCategoryListDOMForEnlargement(false);
    if (listContent) {
...
function openEditModalById(docId) {
    ...
    if (superCategory && DOMESC.editingDocIdInput && DOMESC.editingNameInput && DOMESC.editModal && DOMESC.editingEffectTypesSelector) {
        ...
        populateTagButtonSelector(DOMESC.editingEffectTypesSelector, effectTypeOptionsForButtons, effectTypesInThisSuperCat, 'effectTypeId');
...
// ★★★ saveEffectSuperCategoryEdit 関数の修正 ★★★
async function saveEffectSuperCategoryEdit() {
    ...
    const selectedEffectTypeIds = getSelectedTagButtonValues(DOMESC.editingEffectTypesSelector, 'effectTypeId');
    try {
        ...
        allEffectTypes.forEach(et => {
            ...
            if (isCurrentlySelected && !wasPreviouslyAssigned) {
                ...
                batch.update(effectTypeRef, { superCategoryId: null }); // หรือ deleteField()
            }
        });
        await batch.commit();
